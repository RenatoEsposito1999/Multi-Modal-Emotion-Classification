...
class RAVDESS(data.Dataset):
    ...
    def __str__(self):
        subset_name = self.data[0].get('trainvaltest', '')

        spatial_transform_str = repr(self.spatial_transform)

        return f"RAVDESS Dataset - Subset: {subset_name}, Size: {len(self)}, Spatial Transform: {spatial_transform_str}, Audio Transform: {self.audio_transform}, Data Type: {self.data_type}"